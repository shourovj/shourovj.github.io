jQuery(document).ready(function($) {
    ...
    $(document).on('click', '#DiscussionForm :submit', function() {
        ...
        var draft = $(btn).attr('name') == $('#Form_SaveDraft').attr('name') ? true : false;
        var postValues = $(frm).serialize();
        //Added by Faolan: Check discussion form for sentinel to prevent ajax submission
        //remove sentinel so that it only gets submitted once
        let postValuesArray = $(frm).serializeArray();
        for (v of postValuesArray) {
            if (v.name == "zotero_posthelp_prevent_submit" && v.value == '1') {
                //remove sentinel
				document.querySelector('input[name="zotero_posthelp_prevent_submit"]').remove();
                //return without ajax-submit
                return;
            }
        }
        postValues += '&DeliveryType=VIEW&DeliveryMethod=JSON'; // DELIVERY_TYPE_VIEW
